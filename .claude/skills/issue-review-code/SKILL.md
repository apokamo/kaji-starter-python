...
`/issue-implement` Step 8.5 で生成される `## Pre-Handoff Review` セクション（経路情報を含む）が Issue コメントに存在することを機械的に確認する。**実装側のゲートが skill markdown の誤読・省略によりサイレントにバイパスされていないか**を、レビュー側でハードチェックする責務。
...
> **趣旨**: pre-handoff review の自己評価バイパスを抑える hard boundary。
> 実装側の self-check が機能しないランタイムでも、review-code 側で必ず止まる。
...
2. **Lint / Format / 型チェック（exit 0 必須）**:
   ```bash
   cd [worktree_dir] && source .venv/bin/activate && make lint && ruff format --check src/ tests/ scripts/ && make typecheck
   ```
...
**verdict マーカーの無条件付与（必須）**: 判定コメントには **常に** `--verdict-step review-code --verdict-status <STATUS>` を付与する。`<STATUS>` は本 skill が返す status（`PASS` / `RETRY` / `BACK` / `BACK_IMPLEMENT` / `ABORT`）に置換する。CLI が body 1 行目に `<!-- kaji-verdict: step=review-code status=<STATUS> -->` を決定的に付与し、`issue-design` Step 1.6 の BACK 再入検出はこのマーカーのみを参照する。「BACK のときだけ付ける」条件付き出力は禁止。投稿前に判定を確定してから status を確定すること。
