...
この starter は Python 単一スタックである。baseline artifact が `clean` の場合は以下の 1 本を実行する。
...
| metadata-only / packaging-only | `uv lock --check` または設計書で定めた変更固有検証 |
...
| 変更タイプ固有検証 | PASS / FAIL / N/A | 例: `make verify-docs` / `uv lock --check` |
